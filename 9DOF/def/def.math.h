...
namespace def
{
    ...
    inline arma::mat coord_rotational_matrix(const arma::vec& eulerAngles)
    {
        ...
        tmp(0, 1) = cos(theta) * sin(psi);
        tmp(0, 2) = -sin(theta);

        tmp(1, 0) = sin(phi) * sin(theta) * cos(psi) - cos(phi) * sin(psi);
        tmp(1, 1) = sin(phi) * sin(theta) * sin(psi) + cos(phi) * cos(psi);
        tmp(1, 2) = cos(theta) * sin(phi);

        tmp(2, 0) = cos(phi) * sin(theta) * cos(psi) + sin(phi) * sin(psi);
        tmp(2, 1) = cos(phi) * sin(theta) * sin(psi) - sin(phi) * cos(psi);
