...
class Parafoil
{
    const double COEF_LIFT_BASE = 0.4; //denoted by C_P_L0
    const double COEF_LIFT_ALPHA = 2;//denoted by C_P_LAlpha
    const double COEF_DRAG_BASE = 0.15;//denoted by C_P_D0 changed from 0.15; ?(0.25)
    const double COEF_DRAG_ALPHA = 0;//denoted by C_P_DAlpha
    const double COEF_DRAG_ALPHA_2 = 1;//1; //denoted by C_P_DAlpha

//    const double COEF_LIFT_BASE = 0.5; //denoted by C_P_L0
//    const double COEF_LIFT_ALPHA = 0.7;//denoted by C_P_LAlpha
//    const double COEF_DRAG_BASE = 0.15;//denoted by C_P_D0 changed from 0.15; ?(0.25)
//    const double COEF_DRAG_ALPHA = 0;//denoted by C_P_DAlpha
//    const double COEF_DRAG_ALPHA_2 = 1;//1; //denoted by C_P_DAlpha
    ...
    const double AREA = 1.4; //(1.4)
    const double SPAN = 2.7; //denoted by b (2.49)
    const double CHORD = 0.51;//0.62;//denoted by c (0.62)
    const double FLAP_WIDTH = 0.1;//0.4; //denoted by t (0.4)
    const double ARC_HEIGHT = 0.2;
...
public:
    ...
    arma::vec get_apperent_mass_force(const arma::vec& velocity, const arma::vec& ang_velocity, const arma::vec& euler_angles) const;
